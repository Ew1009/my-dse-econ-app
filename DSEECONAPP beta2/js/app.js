...
function initNav() {
  ...
  NAV.forEach(function(n) {
    h += '<div class="nav-item' + (S.view === n.id ? ' active' : '') + '" data-nav="' + n.id + '"><i class="fas ' + n.icon + '"></i>' + n.label + '</div>';
  });
  sb.innerHTML = h;
...
Sections.dashboard = function(c) {
  ...
  Object.keys(st.topics || {}).forEach(function(k) {
    ...
    if (typeof tk.attempted === 'number' && typeof tk.correct === 'number') {
      ...
      // fallback: estimate questions as 10 per session and derive correct from session %
